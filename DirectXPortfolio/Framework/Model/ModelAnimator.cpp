...
void ModelAnimator::CalculateAttachBoneIndex()
{
	frameBuffer->Apply();
	if (computeBuffer != NULL)
	{
...
void ModelAnimator::ChangeTexture(int index)
{
	for (ModelMesh* mesh : model->Meshes())
		mesh->TransformsSRV(srv[index]);
...
void ModelAnimator::CreateTexture(int index)
{
	
	if (texture[index] != NULL)
	{
		SafeRelease(texture[index]);
		SafeRelease(srv[index]);
	}
	...
	//Create Texture
	{
		...
		Check(D3D::GetDevice()->CreateTexture2D(&desc, subResource, &texture[index]));
	...
	//CreateSRV
	{
		...
		Check(D3D::GetDevice()->CreateShaderResourceView(texture[index], &srvDesc, &srv[index]));
	}
